...
def user_processing(test_user_slug):
    ...
    df_test_merged[list_transform_columns] = df_test_merged[list_transform_columns].apply(stl.transform)
    ...
    X_test_one_user_data = pd.DataFrame(process2.transform(fixed_data.drop('rate', axis=1)))
